...
impl CandidInvoice {
    pub fn into_invoice(self) -> Invoice {
        ...
        Invoice::from_str(&self.invoice).expect("invalid BOLT11 in CandidInvoice")
    }
}

impl From<PayInvoice> for CandidInvoice {
    fn from(p: PayInvoice) -> Self {
        let invoice: Invoice = p.invoice;
        ...
        let payment_hash = invoice.payment_hash().into_inner().to_vec();

        // Payment secret (PaymentSecret([u8; 32]))
        let payment_secret = invoice.payment_secret().0.to_vec();

        // Timestamp: SystemTime -> seconds since UNIX_EPOCH
        let ts: SystemTime = invoice.timestamp();
        let timestamp = ts.duration_since(UNIX_EPOCH).unwrap_or_default().as_secs();
...
mod tests {
    ...
    fn test_invoice_candid_roundtrip() {
        ...
        assert_eq!(
            ...
            original_invoice.payment_hash().into_inner().to_vec()
        );
        assert_eq!(
        ...
        assert_eq!(
            original_invoice.payment_hash().into_inner().to_vec(),
            decoded_invoice.payment_hash().into_inner().to_vec()
        );
        assert_eq!(
            original_invoice.payment_secret().0.to_vec(),
